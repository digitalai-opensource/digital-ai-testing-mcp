...
export interface InspectionSession {
  ...
  // Browser sessions are always W3C.
  sessionFormat: 'jwp' | 'w3c';
  // Target platform — drives element attribute names, launch mechanism, and
  // which keys/controls are available. 'web' = browser Selenium session.
  platform: 'android' | 'ios' | 'web';
  // Set only when platform === 'web' — the browser used for this inspection session.
  browserName?: string;
  // Set only when platform === 'web' — updated by navigate_to.
  currentUrl?: string;
