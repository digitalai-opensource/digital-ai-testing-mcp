...
export function registerReservationTools(server: McpServer): void {
  ...
  server.tool(
    ...
    'Shows the reservation schedule for a specific device over a time window, so you can find an available slot. Cloud Admin only (project keys receive 403). Accepts numeric device ID, serial number, UDID, or device name.',
    {
