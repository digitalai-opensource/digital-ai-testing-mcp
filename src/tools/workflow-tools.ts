  ...
  server.tool(
    ...
    ({ customerName, region, deviceCount, iosCount, androidCount, automationType, salesforceUrl, endDate, users, appName, androidPackageName, iosBundleId }) => {
      ...
      const instructions = [
        ...
        ``,
        `─── STEP 7 — Create or reuse the POC project ────────────────────────────`,
        ...
        `Call: list_projects (you may already have this from Step 7)`,
        `  outputFormat: "json"`,
  ...
  server.tool(
    ...
    ({ customerName }) => {
      ...
      const instructions = [
        ...
        `Flatten all project names across all role keys and remove "${pocName}" from the list.`,
        ``,
  ...
  server.tool(
    ...
    ({ projectName, region, deviceCount, iosCount, androidCount, automationType, projectMemo, isolateDevices, users, appName, androidPackageName, iosBundleId }) => {
      ...
      const instructions = [
        ...
        ``,
        `─── STEP 2 — Create or reuse a device group ─────────────────────────────`,
  ...
  server.tool(
    ...
    ({ projectName }) => {
      ...
      const instructions = [
        ...
        ``,
        `For each returned user, inspect their "roles" field. Flatten all project names`,
        `and remove "${projectName}" from the list.`,
  ...
  server.tool(
    ...
    ({ projectName, confirmDeletion }) => {
      ...
      const instructions = [
        ...
        `     For each user, inspect their "roles" field. Flatten all project names,`,
