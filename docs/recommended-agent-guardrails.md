...
Before you plan, generate, scaffold, or hand-write ANY test for a mobile app on
the Digital.ai platform, decide the mode FIRST — before creating a task list,
before calling any MCP tool, before opening an editor:
...
- Writing or generating a test file with invented/placeholder element IDs
  (`nav_catalog`, `home_container`, …), guessed XPaths, or fabricated credentials.
- Doing the above by authoring the file yourself instead of calling
  `get_test_boilerplate` — bypassing the tool does NOT exempt you from this rule.
...
`start_inspection_session` first. A test-type label from a menu ("login test",
"smoke", "e2e") is a category, not a specification. If unsure which mode the user
wants, ASK before calling any tool or writing any code.
```
