<picture>
  ...
  <img alt="Digital.ai Testing MCP Server" src=".github/images/dai-logo-light.svg" width="600">
</picture>
...
1. Log in to the Digital.ai Testing portal
...
The server supports two test-creation modes for both mobile and web, and the agent is instructed to route between them based on context:

| | **Autonomous** (`get_test_boilerplate` / `get_web_test_boilerplate`) | **Interactive** (`start_inspection_session` / `start_browser_inspection_session` / `collaborative_test_creation` / `collaborative_web_test_creation` prompt) |
|---|---|---|
| **When** | Intent is specific — a standardized flow ("create a login test") or step-level detail — AND selectors are derivable (app source in the workspace, or a prior inspection session) | Request is vague ("I want a test for app X"), no source access (chat-only, or IDE without this app's code), or the user wants to watch/drive on a live device or browser |
| **User involvement** | None required — the agent writes the test from boilerplate + source knowledge + best practices | Collaborative — the agent builds the test step by step with the user (browser sessions relay screenshots; mobile sessions share a live device view URL) |
| **Output** | In an IDE: a local test automation project. Chat-only: portable project files presented inline | Same, plus selectors verified against the real app or page along the way |

**Hybrid is often best:** even in autonomous mode, the agent can open a short inspection session — no user involvement — to capture or verify element IDs or CSS selectors against the real build. The agent is explicitly instructed to **never fabricate selectors** — when in doubt it verifies live, and when the mode itself is ambiguous it asks.

> **Stronger guarantees:** the server ships this routing policy automatically in its MCP `instructions`, and both boilerplate tools structurally refuse to emit code for a real app or URL without verified selectors. Because not every client surfaces server `instructions` — and no MCP server can intercept a file an agent writes by hand — you can install a client-side reinforcement for a stronger, client-independent guarantee. See **[Recommended agent guardrails](docs/recommended-agent-guardrails.md)**.
...
Generates a complete, pre-configured Selenium test script for browser testing. The Digital.ai server URL and access key are pre-filled from the active connection profile.
...
**Recommended workflow for building a new web test:**

1. `list_available_browsers` — confirm which browsers and OS combinations are available in your region
2. `start_browser_inspection_session` — open a live browser session on the target URL
3. `get_page_dom` — extract the rendered DOM, including shadow trees; note element IDs and `data-testid` values
4. `find_web_elements` / `tap_element` / `type_into_element` — interact with elements to verify selectors work
5. `take_inspection_screenshot` — relay screenshots at each key step for verification
6. `stop_browser_inspection_session`
7. `get_web_test_boilerplate(confirmSelectorsVerified: true)` — generate the verified script
...
Use of this software is also subject to the [Supplemental Terms of Use](./SUPPLEMENTAL_TERMS.md), which address AI client responsibilities, agentic action risks, liability limitations, and other obligations specific to the Digital.ai MCP Server for Testing. By using this software, you agree to both the Apache 2.0 License and the Supplemental Terms.